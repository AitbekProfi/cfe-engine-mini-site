(() => {
  ...
  function autoload_() {
    try {
      ...
      state.currentQuestionIndex = Number.isFinite(s.currentQuestionIndex) ? s.currentQuestionIndex : 0;
    } catch (_) {}
  }
  autoload_();
  ...
  function getQuestions_() {
    return window.CFE?.QUESTIONS || [];
  }

  function getAnsweredCount_() {
    return Object.keys(state.answers || {}).length;
  }

  function isCurrentAnswered_() {
    const qs = getQuestions_();
    const q = qs[state.currentQuestionIndex];
    if (!q) return false;
    return Number.isFinite(Number(state.answers[q.id]));
  }

  function updateTestProgressUI_() {
    const qs = getQuestions_();
    const total = qs.length || 1;
    const current = clamp_(state.currentQuestionIndex + 1, 1, total);
    const answered = getAnsweredCount_();
    const pct = Math.round((answered / total) * 100);

    $("#test-progress-text") && ($("#test-progress-text").textContent = `Вопрос ${current} из ${total}`);
    $("#test-answered-badge") && ($("#test-answered-badge").textContent = `${answered} / ${total}`);
    const bar = $("#test-progress-bar");
    if (bar) bar.style.width = `${pct}%`;

    const prevBtn = $("#btn-test-prev");
    const nextBtn = $("#btn-test-next-question");
    if (prevBtn) prevBtn.disabled = state.currentQuestionIndex <= 0;
    if (nextBtn) nextBtn.textContent = current >= total ? "Завершить вопросы" : "Дальше";
  }

  function renderTestQuestion_() {
    const qs = getQuestions_();
    const total = qs.length;
    if (!total) return;

    state.currentQuestionIndex = clamp_(state.currentQuestionIndex, 0, total - 1);

    const q = qs[state.currentQuestionIndex];
    const container = $("#test-question-card");
    if (!container) return;

    const text = state.lang === "kg" ? q.text_kg : q.text_ru;
    const currentValue = Number(state.answers[q.id] || 0);

    container.innerHTML = `
      <div>
        <div class="muted">${q.block}${q.role ? " · " + q.role : ""}</div>
        <h3 class="mt">${q.id}. ${escapeHtml_(text)}</h3>
        <div class="answers-grid">
          ${[1,2,3,4,5].map(v => `
            <button class="btn answer-btn ${currentValue === v ? "btn-primary" : ""}" data-answer="${v}" type="button">${v}</button>
          `).join("")}
        </div>
        <div class="muted mt">Шкала: 1 = совсем не про меня, 5 = очень про меня</div>
      </div>
    `;
    ...
    updateTestProgressUI_();
  }

  $("#btn-test-prev")?.addEventListener("click", () => {
    state.currentQuestionIndex = clamp_(state.currentQuestionIndex - 1, 0, getQuestions_().length - 1);
    autosave_();
    renderTestQuestion_();
  });

  $("#btn-test-next-question")?.addEventListener("click", () => {
    const qs = getQuestions_();
    if (!isCurrentAnswered_()) {
      alert("Сначала ответь на этот вопрос.");
      return;
    }
  ...
  $("#btn-test-to-cr")?.addEventListener("click", tryGoToCR_);

  function tryGoToCR_() {
    const total = getQuestions_().length;
    const answered = getAnsweredCount_();

    if (answered < total) {
      alert(`Ты ответил(а) не на все вопросы. Сейчас отвечено: ${answered} из ${total}.`);
      return;
    }
  ...
  $("#btn-cr-back")?.addEventListener("click", () => {
    showScreen("test");
    renderTestQuestion_();
  });
  ...
  function renderCases() {
    ...
    el.innerHTML = `
      <div class="card">
        <label>Кейс 1 — ситуация и что ты сделал(а)</label>
        <textarea id="case-1" rows="5"></textarea>

        <label class="mt">Кейс 2 — когда было сложно, как ты справился(ась)</label>
        <textarea id="case-2" rows="5"></textarea>

        <label class="mt">Кейс 3 — что у тебя получается лучше всего</label>
        <textarea id="case-3" rows="5"></textarea>
      </div>
    `;
  ...
  $("#btn-cases-back")?.addEventListener("click", () => {
    renderCR();
    showScreen("cr");
  });
  ...
  function hasRole_(picked, key) {
    return (picked || []).some(x => x.key === key);
  }
  ...
  function saveCatalogToCache_(items, warnings) {
    try {
      localStorage.setItem(CATALOG_CACHE_KEY, JSON.stringify({ ts: Date.now(), items, warnings }));
    } catch (_) {}
  }
  ...
  function computeCatalogTopIntoComputed_() {
    ...
    const scored = state.catalog
      .map(item => ({ item, score: scoreProfessionV1_(state.computed, item) }))
  ...
  function renderResult() {
    ...
    const roles = (c.roles_picked || []).map(r =>
      `<span class="pill"><b>${escapeHtml_(roleName_(r.key))}</b> ${r.score}</span>`
    ).join(" ");
    ...
    el.innerHTML = `
      ...
      <div class="card mt"><h3>Кластеры</h3><div class="stack mt">${clusters}</div></div>
      <div class="card mt"><h3>Роли</h3><div class="rowgap mt">${roles}</div></div>
      <div class="card mt"><h3>Блоки</h3><div class="grid mt">${blockItems}</div></div>
